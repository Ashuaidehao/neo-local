...
docker cp configs/v2.10.2/neo-cli.zip $CONTAINER:/opt/
...
for f in configs/v2.10.2/*.json; do echo $f && docker cp $f $CONTAINER:/opt/node/neo-cli/; done
docker cp configs/v2.10.2/Neo.dll $CONTAINER:/opt/node/neo-cli/
